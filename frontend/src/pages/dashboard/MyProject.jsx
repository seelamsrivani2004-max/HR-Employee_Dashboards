                    ...
                    {pending.length > 0 && (
                        <div style={{ marginBottom: '2rem' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                {pending.map(inv => (
                                    <div key={inv.id} className="card" style={{ padding: '1.5rem', borderLeft: '4px solid #f59e0b' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: '0.6rem' }}>
                                                <button onClick={() => respond(inv.id, 'Accepted')} disabled={processing === inv.id}
                                                    style={{
                                                ...
                                                <button onClick={() => respond(inv.id, 'Rejected')} disabled={processing === inv.id}
                                                    style={{
                        ...
                        <div style={{ marginBottom: '2rem' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                {accepted.map(inv => {
                                    ...
                                    const projectIdeas = ideas[project.id] || [];
                                    return (
                                        <div key={inv.id} className="card" style={{ padding: '1.5rem', borderLeft: '4px solid #22c55e' }}>
                                            {/* Project Info */}
                                            ...
                                            <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '1rem' }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '0.75rem' }}>
                                                    <textarea
                                                        value={ideaText[project.id] || ''}
                                                        onChange={e => setIdeaText(prev => ({ ...prev, [project.id]: e.target.value }))}
                                                    ...
                                                    />
                                                    <button onClick={() => submitIdea(project.id)} disabled={postingIdea === project.id || !ideaText[project.id]?.trim()}
                                                        style={{
                                                            ...
                                                            opacity: (!ideaText[project.id]?.trim() || postingIdea === project.id) ? 0.6 : 1, alignSelf: 'flex-start'
                                                        }}>
                                                        <Send size={14} />
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '280px', overflowY: 'auto' }}>
                                                    {projectIdeas.length === 0 ? (
                                                        ...
                                                        <div key={idea.id} style={{ padding: '0.75rem 1rem', background: '#fefce8', borderRadius: '0.5rem', borderLeft: '3px solid #fbbf24' }}>
                    ...
                    {rejected.length > 0 && (
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {rejected.map(inv => (
                                    <div key={inv.id} style={{ padding: '0.85rem 1rem', background: '#fef2f2', borderRadius: '0.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
